document.addEventListener("DOMContentLoaded", function () {
  ...
  function initializeCarousel() {
    ...
    function updateCarouselButtons() {
      ...
      const isAtEnd =
        parksCarousel.scrollLeft + parksCarousel.clientWidth >=
        parksCarousel.scrollWidth - 10;
  ...
  function initializeGallery() {
    ...
    function updateMainImage(index) {
      ...
      if (galleryNextBtn)
        galleryNextBtn.disabled = index === TOTAL_GALLERY_IMAGES;
    ...
    if (galleryNextBtn) {
      galleryNextBtn.addEventListener("click", function () {
        if (currentGalleryIndex < TOTAL_GALLERY_IMAGES)
          updateMainImage(currentGalleryIndex + 1);
      });
    }
    ...
    if (bookingForm) {
      ...
      bookingForm.addEventListener("submit", function (e) {
        if (healthCheckbox && !healthCheckbox.checked) {
          e.preventDefault();
          alert(
            "You must acknowledge and agree to the health & safety guidelines.",
          );
          if (healthLink) {
